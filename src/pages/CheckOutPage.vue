    ...
    <div class="max-w-7xl mx-auto">
      <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
        <div class="flex items-center justify-between">
          <div>
            <h3 class="font-bold text-gray-800 mb-1">Check-Outs del Día</h3>
            <p class="text-gray-600">Gestiona las salidas programadas</p>
          </div>
          ...
          <q-list separator>
            <q-item class="bg-gray-50 text-gray-700 font-semibold">
              ...
              <q-item-section style="max-width: 120px;">
                <div class="text-xs">Hora Salida</div>
              </q-item-section>
              <q-item-section>
            ...
            <q-item v-else-if="checkOuts.length === 0">
              <q-item-section>
                <div class="text-center py-12">
                  <q-icon name="event_busy" size="4em" color="grey-4" class="mb-3" />
                  <div class="text-gray-500 text-lg">No hay check-outs programados para {{ formattedDate }}</div>
                </div>
              </q-item-section>
            </q-item>
            <q-item
              ...
              :class="{ 'bg-blue-50': checkOut.hasArrived }"
            >
              <q-item-section avatar class="min-w-0 pr-0" style="max-width: 50px;">
                <q-checkbox
                  :model-value="checkOut.hasArrived"
                  @update:model-value="toggleDeparture(checkOut)"
                  color="blue"
                  size="md"
                />
              </q-item-section>
              <q-item-section style="max-width: 120px;">
                <div v-if="editingTime === checkOut.id">
                  <q-input
                    :model-value="checkOut.actualTime || checkOut.scheduledTime"
                    mask="##:##"
                    fill-mask
                    dense
                    outlined
                    bg-color="white"
                    @blur="(e) => updateTime(checkOut, e.target.value)"
                    @keyup.enter="(e) => updateTime(checkOut, e.target.value)"
                    autofocus
                  >
                ...
                <div
                  v-else
                  @click="startEditingTime(checkOut.id)"
                ...
                >
                  <div class="flex items-center gap-2">
                    ...
                    <span class="font-mono">{{ checkOut.actualTime || checkOut.scheduledTime || '--:--' }}</span>
      ...
      <div class="mt-6 bg-white rounded-lg shadow-sm p-4">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-6">
            <div>
              <div class="text-sm text-gray-600">Total Check-Outs</div>
              <div class="text-2xl font-bold text-gray-800">{{ checkOuts.length }}</div>
            </div>
            <div>
              <div class="text-sm text-gray-600">Completados</div>
              <div class="text-2xl font-bold text-blue-600">
                {{ checkOuts.filter(c => c.hasArrived).length }}
              </div>
            </div>
            <div>
