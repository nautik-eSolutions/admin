...
onMounted(async () => {
  loading.value = true
  zones.value = await ZoneService.getZonesByPort(PORT_ID)
  loading.value = false
})
...
async function createZone() {
  ...
  try {
    ...
    if (response.status === 200 || response.status === 201) {
      ...
      $q.notify({ type: 'positive', message: 'Zona creada correctamente', position: 'bottom-right' })
...
function confirmDelete(zone) {
  $q.dialog({
    ...
    try {
      ...
      $q.notify({ type: 'positive', message: 'Zona eliminada correctamente', position: 'bottom-right' })
    } catch (error) {
      if (error.response && error.response.status === 409) {
        ...
        $q.notify({ type: 'negative', message: 'Error inesperado al intentar eliminar la zona.' })
  ...
  <q-page padding class="bg-white">
    <div class="main-container">
      ...
      <div class="row items-center justify-between q-mb-xl">
        <div class="text-h5 text-weight-bold">Zonas</div>
      ...
      >
        ...
        <template v-slot:body-cell-actions="props">
          <q-td :props="props" class="q-gutter-x-sm">
            ...
            <q-btn flat dense size="sm" color="negative" icon="delete" label="Eliminar" @click="confirmDelete(props.row)" />
          </q-td>
        </template>
